...
import { GoogleSyncConfig, UserRole, AppUser, RolePermissionsMap } from '../types';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const perms = rolePermissions[currentRole] || rolePermissions.ADMIN;
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2.5 overflow-hidden">
          ...
          {!isCollapsed && (
            <div className="truncate">
              <div className="font-bold text-sm leading-none text-white flex items-center gap-1.5">
                <span>IMS PRO</span>
                <span className="text-[9px] px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 font-bold border border-blue-500/30">
                  v2.4
                </span>
              </div>
              <p className="text-[10px] text-slate-400 mt-1 truncate">Quản Lý Nhập Xuất Tồn</p>
            </div>
          )}
        </div>
        <button
      ...
      <div className="p-3 space-y-2 flex-1 overflow-y-auto">
        ...
        {perms.canCreateVoucher && (
          <>
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-emerald-950/40 hover:bg-emerald-900/60 border border-emerald-800/60 text-emerald-300 text-xs font-bold transition-all cursor-pointer shadow-sm ${
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-blue-950/40 hover:bg-blue-900/60 border border-blue-800/60 text-blue-300 text-xs font-bold transition-all cursor-pointer shadow-sm ${
                isCollapsed ? 'justify-center px-0' : ''
              }`}
        ...
        {perms.canSyncSheets && googleConfig.spreadsheetId && (
          <button
            onClick={onQuickSync}
            disabled={isSyncing}
      ...
      {/* User Info Card */}
      {currentUser && (
        <div className={`mt-auto border-t border-slate-800/80 bg-slate-950/80 p-3 ${isCollapsed ? 'text-center' : ''}`}>
          {!isCollapsed ? (
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-mono text-blue-300 font-bold truncate max-w-[130px]" title={currentUser.email}>
                  {currentUser.email}
                </span>
                <span
