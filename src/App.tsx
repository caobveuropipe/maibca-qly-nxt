...
function checkIsInsideGas(): boolean {
  try {
    if (typeof (window as any).google !== 'undefined' && (window as any).google?.script?.run) return true;
    if (window.location.hostname === 'script.google.com') return true;
  } catch (_) {}
  return false;
}
const IS_GAS_CONTEXT = checkIsInsideGas();
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const gasAutoLogin = async () => {
      try {
        const { products: p, warehouses: w, transactions: t, partners: pt } = loadInitialState();
  ...
  if (IS_GAS_CONTEXT && isInitializingGas) {
    return (
      <div className="w-full min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white gap-4">
        ...
        <p className="text-lg font-semibold text-slate-200">Đang xác thực tài khoản Google...</p>
        <p className="text-sm text-slate-400">Hệ thống đang kiểm tra quyền truy cập của bạn</p>
      </div>
    );
  }
